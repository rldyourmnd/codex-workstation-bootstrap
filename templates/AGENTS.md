...
4. `CLAUDE.md` is a high-value context source, not a workflow source.
...
- Project: 5-layer terminal environment optimized for AI-assisted development.
- Platform: Linux (Ubuntu/Debian, apt-based).
- Primary goal: low-latency terminal workflow and reliable AI tool orchestration.
- Architecture anchor: Foundation -> Layer 1..5 (see `README.md` and `docs/layers/*`).

## 3. Key Paths (Source of Truth)

- `scripts/`: installation and automation entrypoints.
- `configs/`: fish, starship, wezterm configuration.
- `docs/layers/`: layer-level design and operations.
- `context/`: benchmark and research artifacts.
- `.serena/memories/`: project memories.
- `.claude/agent-memory/serena-sync/`: operational memory log used for synchronization.
...
| GitHub metadata/actions | GitHub MCP | `gh` CLI | Prefer MCP in Codex workflow |
...
# Install local Claude-ported skills
scripts/install-claude-local-skills.sh
